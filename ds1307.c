...
void RTC_SetDateTime(unsigned char h, unsigned char m, unsigned char s,
    ...
    I2C_Master_Start();
    I2C_Master_Write(DS1307_ADDR);  // Direcci n del DS1307 para escritura
    ...
    I2C_Master_Write(DEC2BCD(1));   // D a de la semana (fijo en 1)
    I2C_Master_Write(DEC2BCD(d));   // D a
    I2C_Master_Write(DEC2BCD(mo));  // Mes
    I2C_Master_Write(DEC2BCD(y));   // A o
    I2C_Master_Stop();
}
...
void RTC_GetDateTime(unsigned char *hour, unsigned char *min, unsigned char *sec,
    ...
    *hour  = BCD2DEC(I2C_Master_Read(1));
    I2C_Master_Read(1); // D a de la semana (omitido)
